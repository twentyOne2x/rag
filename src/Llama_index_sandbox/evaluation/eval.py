...
def get_or_create_index(params: Dict[str, Any]) -> Tuple[Any, ServiceContext]:
    ...
    if recreate_index:
        model_details = (
            ...
            params["text_splitter_chunk_size"],
            params["text_splitter_chunk_overlap_percentage"],
        )
        index = create_index(
...
def run(config: Config):
    ...
    for index_comb in config.get_full_combinations():
        ...
        index_params = config.get_index_params(
            text_splitter_chunk_size,
            text_splitter_chunk_overlap_percentage,
            embedding_model_name,
            embedding_model,
        ...
        for llm_model_name, similarity_top_k in product(config.INFERENCE_MODELS, config.NUM_CHUNKS_RETRIEVED):
            ...
            retrieval_engine, query_engine, store_response_partial = initialise_chatbot(
                ...
                query_engine_as_tool=config.query_engine_as_tool,
                index=index,
                service_context=service_context,
            ...
            ask_questions(
                input_queries=EVALUATION_INPUT_QUERIES,
